...
class Projects extends Security_Controller {
    ...
    function task_view($task_id = 0) {
        ...
        // var_dump($this->can_edit_tasks()); die;
        $view_data['can_edit_tasks'] = $this->can_edit_tasks();
        ...
        $view_data['current_user'] = $this->Users_model->get_users_by_id($this->Users_model->login_user_id())[0];
    ...
    private function _initialize_all_related_data_of_project($project_id = 0, $collaborators = "", $task_labels = "") {
        ...
        if ($project_id) {
            ...
            $view_data['assign_to_dropdown'] = $this->Users_model->get_assign_users();
            // $view_data['collaborators_dropdown'] = $related_data["collaborators_dropdown"];
            $view_data['collaborators_dropdown'] = $this->Users_model->get_collaborator_users();
    ...
    function get_all_related_data_of_selected_project($project_id) {
        ...
        if ($project_id) {
            ...
            echo json_encode(array(
                ...
                "assign_to_dropdown" => $this->Users_model->get_assign_users(),
                "collaborators_dropdown" => $this->Users_model->get_collaborator_users(),
    ...
    private function _make_task_row($data, $custom_fields) {
        ...
        if (isset($data->unread) && $data->unread && $data->unread != "0") {
            ...
            $icon = "<i data-feather='message-circle' class='icon-16'></i>";
        }
        ...
        $assigned_to = $this->_get_assigned_to_html($data->assigned_to);

        if (!$assigned_to) {
            $assigned_to = "-";
        }

        // print_r($this->Users_model->get_users_by_id($this->Users_model->login_user_id())); die;
        // echo '<pre>';
        // var_dump($data->assigned_to); 

        // print_r($assigned_to); die;
    ...
    private function _get_assigned_to_html($ids, $clickable = true) {
        // var_dump(explode(',', $ids)); 
        $assigned_to_list = $this->Users_model->get_users_by_id($ids);
        $assigned_to_list_html = "";
        if ($assigned_to_list) {
            
            foreach ($assigned_to_list as $AU) {
            // print_r($assigned_to_list); die;
                // $collaborator_parts = explode("--::--", $collaborator);

                $uID = get_array_value($AU, 'id');
                $uName = get_array_value($AU, 'name');

                $image_url = get_avatar(get_array_value($AU, 'image'));
                $user_type = get_array_value($AU, 'user_type');

                $uImage = "<span class='avatar avatar-xs mr10'><img src='$image_url' alt='...'></span>";

                if ($clickable) {
                    if ($user_type == "staff") {
                        $assigned_to_list_html .= get_team_member_profile_link($uID, $uImage, array("title" => $uName));
                    } else if ($user_type == "client") {
                        $assigned_to_list_html .= get_client_contact_profile_link($uID, $uImage, array("title" => $uName));
                    }
                } else {
                    $assigned_to_list_html .= "<span title='$uName'>$uImage</span>";
                }
            }
        }
        return $assigned_to_list_html;
    }
