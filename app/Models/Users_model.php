...
class Users_model extends Crud_model {
    ...
    public function  get_assign_users()
    {
        $sql = "SELECT users.id, CONCAT(first_name, ' ', last_name, '- ', job_title) as text 
                FROM users
                WHERE users.deleted=0 AND users.user_type='staff' AND users.role_id IN (3, 7, 9, 10, 11, 12, 13, 14) 
                ORDER BY users.id DESC";
        return $this->db->query($sql)->getResultArray();
    }
    public function  get_collaborator_users()
    {
        // print_r($this->login_user_id()); die;
        $cUser = $this->get_users_by_id($this->login_user_id())[0];


        $sql = "SELECT users.id, CONCAT(first_name, ' ', last_name, '- ', job_title) as text 
                FROM users
                WHERE users.deleted=0 AND users.user_type='staff' && users.is_admin=0";
        if (in_array($cUser['role_id'], [9, 10, 11, 12, 13])) {
            $sql .= " && users.role_id=6 ";
        }   
    ...
    public function  get_users_by_id($ids)
    {
        ...
        $sql = "SELECT users.id, CONCAT(first_name, ' ', last_name) as name, image, user_type, role_id 
                FROM users WHERE users.deleted=0 AND users.user_type='staff' && users.id IN ($ids)
                ORDER BY users.id DESC";
